...
import { DatePicker, DateTimePicker, MobileDateTimePicker, MobileDateTimePickerProps } from "@mui/x-date-pickers";
...
export type DateInputType = propsProp & MobileDateTimePickerProps;
const DateTimeInput = ({ name, label, isFullPage = true, ...props }: DateInputType) => {
    ...
    <LocalizationProvider key={name} dateAdapter={AdapterMoment}>
      {isFullPage ? (
        <MobileDateTimePicker
          ...
          onChange={(newValue) => {
            setFieldValue(field.name, moment(newValue).utc().toDate());
          }}
          slotProps={{
            textField: {
